...
(*Marc Sulsenti Homework 6 
   Extra Credit 
*)

  (*Overloading Mangle*)
  (*PART 2, replace where needed*)
...
(*in_subclass extension*)
let is_subclass : string -> string -> class_env -> exp_val ea_result = 
  fun c_name1 c_name2 c_env ->
    let rec helper c_name acc =
      match List.assoc_opt c_name c_env with
      | None -> acc
      | Some (super, _fields, _methods) ->
          helper super (super :: acc)
    in
    if List.mem_assoc c_name2 c_env then
      let ancestors = helper c_name1 [c_name1] in
      return (BoolVal (List.mem c_name2 ancestors))
    else
      error ("is_subclass: class " ^ c_name2 ^ " not found")
  ...
  | NewObject(c_name,es) ->
    ...
    (match List.assoc_opt c_name !g_class_env with
     ...
     | Some (_super,fields,methods) -> 
       ...
       (*Mangle*)
       in let init = (name_mangle "initialize" args)
       in (match List.assoc_opt init methods with
           | None -> return self
           | Some m -> apply_method init self args m >>= fun _ ->
             return self))
  | Send(e,m_name,es) ->
    eval_expr e >>= fun self ->
    obj_of_objectVal self >>= fun (c_name,_) ->
    eval_exprs es >>= fun args ->
      let send_name = (name_mangle m_name es ) in
    (match lookup_method c_name send_name !g_class_env with
     | None -> error "Method not found"
     | Some m -> apply_method send_name self args m)
  | Self ->
    eval_expr (Var "_self")
  | Super(m_name,es) ->
    eval_exprs es >>= fun args ->
    ...
    eval_expr (Var "_self") >>= fun self ->
      (*Mangle*)
      let super_name = (name_mangle m_name es) in
    (match lookup_method c_name super_name !g_class_env with
     | None -> error "Method not found"
     | Some m -> apply_method super_name self args m)
  (* List operations* *)
  | List(es) ->
    eval_exprs es >>= fun args ->
    return @@ ListVal args
